...
/**
 * Zeichnet auf jeder Fingerspitze eine zufällige orangefarbene Form.
 * Die Formen haben eine Deckkraft (Alpha) von 20.
  ...
  for (let i = 0; i < hands.length; i++) {
    ...
    for (let tipIdx of fingertipIndices) {
      ...
      if (keypoint) {
        ...
        // Orangefarbe mit Alpha 20
        fill(255, 165, 0, 20); // Orange (255, 165, 0) mit Alpha 20
        noStroke();

        // Wähle zufällig eine Form
        let shapeType = floor(random(3)); // 0: Kreis, 1: Rechteck, 2: Dreieck
...
/**
 * Hilfsfunktion zum Zeichnen eines Dreiecks
 * @param {number} x - x-Position des Mittelpunkts
 * @param {number} y - y-Position des Mittelpunkts
 * @param {number} size - Größe des Dreiecks
 */
function drawTriangle(x, y, size) {
  triangle(
    x, y - size,           // oben
    x - size, y + size,    // unten links
