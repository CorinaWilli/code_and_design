...
function draw() {
  ...
  // --- Berechnung der Maus-basierten Drehgeschwindigkeit ---
  let maxDist = dist(width / 2, height / 2, 0, 0);
  let aktuelleDist = dist(mouseX, mouseY, width / 2, height / 2);

  // Nähe zur Mitte steuert Geschwindigkeit (verstärkter Effekt)
  // Mitte = hohe Geschwindigkeit (~20), Rand = 0
  let speedFactor = map(aktuelleDist, 0, maxDist, 20, 0);
  winkel += speedFactor; // schnellerer Dreheffekt
  ...
  // Farbverlauf erzeugen (leicht transparent)
